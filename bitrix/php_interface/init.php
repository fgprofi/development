	...
	if ( CModule::IncludeModule( "iblock" ) ) {
		if ( isAdministrator() ) {
			...
			$arFilter = array(
				"IBLOCK_ID"                                  => $ib,
				"ACTIVE"                                     => "Y",
				"!PROPERTY_VERIFICATION_PASSED_BY_MODERATOR" => $arVal[ $ib ]
			);
...
function needAuth( $redir, $legal_face_id = false ) {
	...
	if ( ! $USER->IsAuthorized() ) {
		...
		if ( isAdministrator() ) {
			$uID = CIBlockElement::getList( array(), array(
				"IBLOCK_ID" => 7,
				"ID"        => $_GET['id']
			), false, false, array( "PROPERTY_USER_ID" ) )->Fetch()['PROPERTY_USER_ID_VALUE'];
			if ( ! $uID ) {
				$uID = CIBlockElement::getList( array(), array(
					"IBLOCK_ID" => 8,
					"ID"        => $_GET['id']
	...
	function OnAfterUserRegisterHandler( &$arFields ) {
		...
		if ( $arFields['ID'] > 0 ) {
			if ( CModule::IncludeModule( "iblock" ) ) {
				...
				$obRes = CIBlockElement::GetList( array(), array(
					'IBLOCK_ID' => $arIblock[ $_POST["FACE"] ]["U_IBLOCK_ID"],
					"ID"        => $_POST['ID_USER_PR']
				) )->GetNextElement();
				if ( $obRes ) {
