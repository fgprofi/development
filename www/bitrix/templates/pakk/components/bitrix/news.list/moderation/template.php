...
foreach ($arResult['ITEMS'] as $key => $value) {
    $arFilterUsers[] = $value["PROPERTIES"]["USER_ID"]["VALUE"];
}
$data = CUser::GetList(($by="ID"), ($order="ASC"),
    array(
        'ID' => implode("|", $arFilterUsers)
    ),
    array(
        "FIELDS"=>array(
            "ID",
                    ...
                    <div class="profile-filter profile__item">
                        <div class="profile-filter__image">
                            <div class="header-login__img-wrap">
                                ...
                                <span class="header-login__initials<?if($arUsers[$item["PROPERTIES"]["USER_ID"]["VALUE"]]["ACTIVE"]=="N"){ echo ' unconfirmed';}?>"><?= mb_strtoupper(mb_substr($props['SURNAME']['VALUE'],0,1)).' '.mb_strtoupper(mb_substr($props['FIRST_NAME']['VALUE'],0,1))?></span>
                            </div>
                        </div>
