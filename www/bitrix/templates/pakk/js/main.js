...
$(document).ready(function () {
    ...
    function checkValidGroup(target) {
        ...
        var groupPowCountInput = 0;
        parent.find("select, input, textarea").each(function () {
            if (!$(this).hasClass("noCheckGroup") && !$(this).parents(".groop_field-wrap").hasClass("clone-box-group")) {
                if($(this).parents(".groop_field").hasClass("groop_field--work")){
                    if(groupPowCountInput == 4){
                        return;
                    }
                    groupPowCountInput++;
                }
    ...
    $("form[name=add_legal_form]").submit(function(e){
        e.preventDefault;
        var form = $(this);
        if(form.hasClass("wait-send")){
            return false;
        }
        form.addClass("wait-send");
        var er = false;
        ...
        if (!er) {
            $.ajax({
                type: "POST",
                url: "/ajax/add_legal_profile.php",
                data: form.serializeArray(),
                dataType: "html",
                success: function (responseData) {
                    if(responseData){
                        $.fancybox.close();
                        $.fancybox.open('<div id="success-message" class="success-message"><img src="/bitrix/templates/pakk/img/message-success.png" alt="Юр. лицо создано" class="success-message__img"><p class="success-message__text">Юр. лицо создано</p></div>');
                        $.ajax({
                            type: "POST",
                            url: "/ajax/legal_faces_list.php",
                            dataType: "html",
                            success: function (responseData) {
                                $(".profile-moderation__list").html(responseData);
                            }
                        });
                    }
                    console.log(responseData);
    ...
    $('.header-login__btn').on('click', function() {
        $('.header-login__content').fadeToggle();
    });
