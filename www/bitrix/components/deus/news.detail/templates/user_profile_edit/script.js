$(document).ready(function () {
	...
	$(".groop_field--work .multi_field_text_plus").on("click", function(){
		var parentGroup = $(this).parents(".groop_field--work");
		var error = false;
		parentGroup.find(".groop_field-wrap").each(function(){
			if($(this).hasClass("clone-box-group")){
				return;
			}
			$(this).find(".groop_field-input-wrap input").each(function(){
		...
		if(!error){
			var cloneFields = parentGroup.find(".clone-box-group").clone().removeClass("clone-box-group");
			parentGroup.find(".groop_field-wrap").last().after(cloneFields);
			var countGroup = parentGroup.find(".clone-box-group input[name^=POW_NEW]").val();
			var newCountGroup = Number(countGroup)+1;
			parentGroup.find(".clone-box-group input").each(function(){
				$(this).attr("name", $(this).attr("name").replace(countGroup,newCountGroup));
			});
			parentGroup.find(".clone-box-group input[name^=POW_NEW]").val(newCountGroup);
		}
	});
	...
	function checkPOWGroup(groups){
		var error = false;
		groups.find("input").removeClass("error");
		groups.find("input[name^=POW_NEW]").each(function(){
			if($(this).parents(".clone-box-group").length){
				return;
			}
			var group = $(this).parents(".groop_field-wrap");
			var groupData = groups.find("input[name*="+$(this).val()+"]").serializeArray();
			var checkNoEmpty = false;
			for (var i = groupData.length - 1; i >= 0; i--) {
				if(groupData[i]["name"] != "POW_NEW["+$(this).val()+"]"){
					if(groupData[i]["value"] == ""){
						var inputName = groupData[i]["name"].replace("["+$(this).val()+"]", "");
						group.find("input[name^="+inputName+"]").addClass("error");
						error = true;
					}else{
						checkNoEmpty = true;
					}
	...
	$(".cabinet-block-form-form .button-green").click(function (e) {
		...
		if($(this).parents("form").find(".groop_field.groop_field--work").length){
			if(checkPOWGroup($(this).parents("form").find(".groop_field--work"))){
				var res = [];
				res.push("PROPERTY_PLACES_OF_WORK");
			}
		}
		if (res == 'ok') {
