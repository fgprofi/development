...
<div class="containered">
    ...
    <div class="sidebar">
        ...
        <ul class="sidebar__list ul_border_top">
            ...
            <li class="sidebar__item">
                <a class="sidebar__link" href="/admin/subscribe/">Рассылка</a>
            </li>
            <li class="sidebar__item">
                <a class="sidebar__link logout_href" href="">Выход</a>
            </li>
    ...
    <div class="main-content">
        <div class="cabinet-user flex">
            ...
            <div class="cabinet-user__load-photo_box">
                <div class="cabinet-user__load-photo<? if (isset($arResult[" DISPLAY_PROPERTIES"]["PHOTO"]['FILE_VALUE']['ID'])) { echo ' check_logo' ; } ?>" style="background: url('<?= $file['src'] ?>')">
                    <div class="remove_photo<? if (!$arResult[" DISPLAY_PROPERTIES"]["PHOTO"]['FILE_VALUE']['ID']) { echo ' hide' ; } ?>" data-ib="<?= $arResult["IBLOCK_ID"] ?>"></div>
                ...
                <div class="input_box personal_logo">
                    <div class="logo_text add_file">
                        <form action="/ajax/save_photo.php/?<?= $arResult["ID"] ?>" method="get">
                            ...
                            <? // кнопка прокликивается в файле reestr/public_html/bitrix/templates/pakk/components/bitrix/main.file.input/.default/script.js 371 строка?>
                        </form>
                    </div>
                </div>
            </div>
        ...
        <form action="" class="ps_center cabinet-block-form-form js-form-validate">
            <div class="cabinet-block-form">
                ...
                <div class="groop_field block_<?= $a ?>">
                    ...
                    <div class="groop_field_box">
                        ...
                        <div class="flex">
                            <div class="form_input input_box" id="<?= $prop_name ?>">
                                <label for="<?= $prop_name ?>"><?= $arProp["NAME"] ?>
                                    <? if ($arProp['IS_REQUIRED'] == "Y")
                                                            echo '<sup>*</sup>' ?></label>
                                <textarea placeholder="<?= $placeholder ?>" name="<?= $prop_name ?>" <? if ($arProp['IS_REQUIRED']=="Y" ) echo ' required ' ?>><?= $InpVal ?></textarea>
                            </div>
                            <?// echo "<pre>"; print_r($arProp); echo "</pre>";?>
                            <div class="help-empty"></div>
                        </div>
                        ...
                        <div class="flex">
                            <div class="form_input input_box" id="<?= $prop_name ?>">
                                <label for="<?= $prop_name ?>"><?= $arProp["NAME"] ?>
                                    <? if ($arProp['IS_REQUIRED'] == "Y")
                                                            echo '<sup>*</sup>' ?></label>
                                <input <? if ($arProp['IS_REQUIRED']=="Y" ) echo ' required ' ?>
                        ...
                        <div class="flex">
                            <div class="input_box multi_field_text" id="<?= $prop_name ?>">
                                ...
                                <div class="multi_field_text_input valid_minus margin-bottom">
                                    <div class="field-row">
                                        <input type="text" name="<?= $prop_name ?>[<?= $val ?>]" value="<?= $val ?>"><?= $valText ?>
                                    ...
                                    <div class="multi_field_text_input valid_minus">
                                        <? if ($arProp['SELECTED']) {
                                                                    ...
                                                                    if(!empty($arFilterSubDiv)){
                                                                        $iterator = CIBlockElement::GetPropertyValues(8, array('ID' => $arFilterSubDiv), true, array('ID' => array(107)));

                                                                        while ($row = $iterator->Fetch())
                                                                        {
                                                                            $arResSubDiv[$row["IBLOCK_ELEMENT_ID"]] = $row[107];
                                        ...
                                        <div class="field-row auth_legal_face_box">
                                            <div class="select_only_val">
                                                <div class="auth_legal_face_title"><?= $one_sel['NAME'] ?><br><span>Подразделение: <span class="subdivision_name"><?= $arResSubDiv[$one_sel['ID']] ?><span><span></div>
                        ...
                        <?if($arProp["CODE"] == "PLACES_OF_WORK"):?>
                            <?
                                ...
                                $arSelectPOW = Array("ID", "IBLOCK_ID", "NAME", "PROPERTY_*");
                                $arFilterPOW = Array("IBLOCK_ID"=>IntVal($arProp["LINK_IBLOCK_ID"]),"ACTIVE"=>"Y","ID"=>$arProp["~VALUE"]);
                                $resPOW = CIBlockElement::GetList(Array(), $arFilterPOW, false, false, $arSelectPOW);
                                while($obPOW = $resPOW->GetNextElement())
                                {
                                    $arDataPOW["FIELDS"] = $obPOW->GetFields();
                                    foreach ($obPOW->GetProperties() as $dataPropPOW) {
                                        $arDataPropPOW[$dataPropPOW["ID"]] = $dataPropPOW;
                                    }
                                    foreach ($arSortPropPOW as $idPropPow) {
                                        $arDataPOW["PROPS"][$arDataPropPOW[$idPropPow]["CODE"]] = $arDataPropPOW[$idPropPow];
                                    }
                                    $arPow[$arDataPOW["FIELDS"]["ID"]] = $arDataPOW;
                                    //pre($arPropsPOW);
                                }
                            ?> 
                            <?//pre($arPow);?>
                            <div class="groop_field groop_field--work" id="PROPERTY_PLACES_OF_WORK">
                                <?if(!empty($arPow)):?>
                                    <?foreach($arPow as $idPow => $arValPOW):?>
                                        <div class="groop_field-wrap groop_field-wrap--allotted">
                                            <input type="hidden" name="PROPERTY_<?=$arProp["CODE"]?>[]" value="<?=$idPow?>">
                                            <?foreach($arValPOW["PROPS"] as $propCodePOW => $propPOW):?>
                                                <?if($propPOW["LIST_TYPE"] == "L"):?>
                                                    <div class="groop_field-item">
                                                        <div class="label_box">
                                                            <label for=""><?=$propPOW["NAME"]?></label>
                                                        </div>
                                                        <div class="groop_field-input-wrap">
                                                            <input name="<?=$arProp["CODE"]?>[<?=$idPow?>][<?=$propCodePOW?>]" type="text" class="groop_field-input" value="<?=$propPOW["VALUE"]?>" placeholder="<?=$propPOW["HINT"]?>">
                                                        </div>
                                                    </div>
                                                <?endif;?>
                                            <?endforeach;?>
                                ...
                                <?else:?>
                                    <div class="groop_field-wrap groop_field-wrap--allotted">
                                        <input type="hidden" name="POW_NEW[0]" value="0">
                                        <div class="groop_field-item">
                                            <div class="label_box">
                                                <label for="">Название организации</label>
                                            </div>
                                            <div class="groop_field-input-wrap"><input name="POW_ORGANIZATION[0]" type="text" class="groop_field-input" placeholder="ООО “Рога и Копыта”"></div>
                                        </div>
                                        <div class="groop_field-item">
                                            <div class="label_box">
                                                <label for="">Организационно - правовая форма</label>
                                            </div>
                                            <div class="groop_field-input-wrap"><input name="POW_ORGANIZATIONAL_AND_LEGAL_FORM[0]" type="text" class="groop_field-input" placeholder="ООО"></div>
                                        </div>
                                        <div class="groop_field-item">
                                            <div class="label_box">
                                                <label for="">Должность</label>
                                            </div>
                                            <div class="groop_field-input-wrap"><input name="POW_POSITION[0]" type="text" class="groop_field-input" placeholder="Консультант по финансам"></div>
                                        </div>
                                        <div class="groop_field-item">
                                            <div class="label_box">
                                                <label for="">Вид деятельности</label>
                                            </div>
                                            <div class="groop_field-input-wrap"><input name="POW_KIND_OF_ACTIVITY[0]" type="text" class="groop_field-input" placeholder="Методист"></div>
                                        </div>
                                    </div>
                                <?endif;?>
                                <div class="groop_field-wrap groop_field-wrap--allotted clone-box-group">
                                    <input type="hidden" name="POW_NEW[1]" value="1">
                                    <div class="groop_field-item">
                                        <div class="label_box">
                                            <label for="">Название организации</label>
                                        </div>
                                        <div class="groop_field-input-wrap"><input name="POW_ORGANIZATION[1]" type="text" class="groop_field-input" placeholder="ООО “Рога и Копыта”"></div>
                                    </div>
                                    <div class="groop_field-item">
                                        <div class="label_box">
                                            <label for="">Организационно - правовая форма</label>
                                        </div>
                                        <div class="groop_field-input-wrap"><input name="POW_ORGANIZATIONAL_AND_LEGAL_FORM[1]" type="text" class="groop_field-input" placeholder="ООО"></div>
                                    </div>
                                    <div class="groop_field-item">
                                        <div class="label_box">
                                            <label for="">Должность</label>
                                        </div>
                                        <div class="groop_field-input-wrap"><input name="POW_POSITION[1]" type="text" class="groop_field-input" placeholder="Консультант по финансам"></div>
                                    </div>
                                    <div class="groop_field-item">
                                        <div class="label_box">
                                            <label for="">Вид деятельности</label>
                                        </div>
                                        <div class="groop_field-input-wrap"><input name="POW_KIND_OF_ACTIVITY[1]" type="text" class="groop_field-input" placeholder="Методист"></div>
                                    </div>
                                </div>
                                <div class="multi_field_text_plus">Добавить место работы
                                    <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M15.8244 8.71775H1M8.29893 15.8244V1" stroke="#1D2220" stroke-opacity="0.4" stroke-width="2" stroke-linecap="round" />
                                    </svg>
                                </div>
                                <div class="input_box checkbox">
                                    ...
                                    <input type="checkbox" <?= $check ?> value="119" name="HIDDEN_VIEW[119]" id="PLACES_OF_WORK">
                                    <label for="PLACES_OF_WORK">Информация блока “Место работы” будет видна всем участникам Сообщества</label>
                                </div>
                            </div>
                            <?continue;?>
                        <?endif;?>
                            ...
                            <div class="groop_ta">
                                ...
                                <div class="input_box" id="<?= $prop_name ?>">
                                    <div class="label_box" data-s="<?= $arProp['REPRESENTATIVE_OF_LEGAL_FACES'] ?>">
                                        <label for="<?= $arProp["CODE"] ?>"><?= $arProp["NAME"] ?>
                                            <? if ($arProp['IS_REQUIRED'] == "Y" || $arProp["CODE"] == "KIND_OF_ACTIVITY")
                                                            echo '<sup>*</sup>' ?></label>
                                    </div>
                                    <?
                                                $noCheckGroup = "";
                                    ...
                                    <div class="select input_box form_input">
                                        <select <? if ($arProp['IS_REQUIRED']=="Y" || $arProp["CODE"]=="KIND_OF_ACTIVITY" ) echo ' required ' ?>
                                            ...
                                            <? foreach ($arOptions as $opt):
                                                                ...
                                                                ?>
                                            <option value="<?= $opt["ID"] ?>" <?= $selected ?> data-name="<?= $opt["NAME"] ?>"><?= $opt["NAME"] ?> <?= $ifUser ?></option>
                                            <? endforeach; ?>
                                        </select>
                                    </div>
                                    <!--                                                -->
                    ...
                    <div class="input_box" id="<?= $prop_name ?>">
                        <div class="label_box">
                            <label for="<?= $arPropGroop["CODE"] ?>"><?= $arPropGroop["NAME"] ?>
                                <? if ($arProp['IS_REQUIRED'] == "Y")
                                                            echo '<sup>*</sup>' ?></label>
                        </div>
                        <input <? if ($arProp['IS_REQUIRED']=="Y" ) echo ' required ' ?> type="text"
                    ...
                    <? endif; ?>
                    <? if ($arProp["ID"] && $arProp["ID"] != 21 && $arProp["ID"] != 23 && $arProp["ID"] != 103): ?>
                    <?
                                    $disabled_check = "";
                    ...
                    <div class="input_box checkbox<?= $disabled_check ?>">
                        <?
                                        ...
                                        if (!in_array($arProp["ID"], $arPropHidden)):?>
                        <? $check = ' checked="checked"'; ?>
                        <input type="checkbox" <?= $check ?> value="<?= $arProp["ID"]; ?>" name="HIDDEN_VIEW[<?= $arProp["ID"]; ?>]" id="VIEW_<?= $prop_name ?>">
                        <? else: ?>
                        <input type="checkbox" value="<?= $arProp["ID"]; ?>" name="HIDDEN_VIEW[<?= $arProp["ID"]; ?>]" id="VIEW_<?= $prop_name ?>">
                        <? endif; ?>
