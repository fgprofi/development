...
//		die();
		...
		foreach ( $arData['id'] as $usr ) {
			...
			if ( $moderator != '' && $moderator >= 0 ) {
				...
				$event->sendModerateUser( array( 'fields' => $fields, 'props' => $props ) );
				$redirect_to = $arRedirect[$fields['IBLOCK_ID']];
			}

			//признак удаления карточки
			if ( $props['SIGN_OF_USER_DATA_DELETION']['VALUE_ENUM_ID'] != 3 && $trash == 'true' ) {
				$obEl->SetPropertyValuesEx( $usr, $fields['IBLOCK_ID'], array( 'SIGN_OF_USER_DATA_DELETION' => 3 ) );
